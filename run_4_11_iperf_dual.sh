...
INTERVAL="${INTERVAL:-5}"
SERVER_LOG="${SERVER_LOG:-/tmp/iperf_4_11_server.log}"
...
run_client_test() {
  ...
  echo
  iperf -c "${SERVER_IP}" -t "${TEST_DURATION}" -i "${INTERVAL}" -d
}

echo "4.11 Ethernet iperf dual test"
echo "Host: $(hostname)"
...
echo "Summary"
echo "-------"
echo "Review the iperf client output above for simultaneous dual-direction bandwidth results."
